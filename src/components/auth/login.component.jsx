...
export const LoginComponent = ({ navigation }) => {
    ...
    const SignIn = async (email, password) => {
        try {
            ...
            setLoading(true);
            await signInWithEmailAndPassword(authentication, email, password).then((result) => { setUser(result._tokenResponse.email); setLoading(false) }).catch((error) => { setLoading(false); alert( 'auth/user-not-found' in error ? "please sign up" : "email or password doesn't exist"); })
        } catch (error) {
            setLoading(false);
            alert(error.message);
        }
